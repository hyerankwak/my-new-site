...
function worksheetBody(item) {
  ...
  <section id="download"><h2>PDF 다운로드</h2><p>현재 자료실의 PDF 파일은 내용과 인쇄 상태를 점검한 뒤 순차적으로 연결합니다. 아래 버튼은 자료가 준비 중임을 알리는 표시입니다.</p><p><a class="button primary" href="#" aria-disabled="true">PDF 다운로드 준비 중</a></p></section>
  ${faqHtml(item.title, item.faq)}`;
}
