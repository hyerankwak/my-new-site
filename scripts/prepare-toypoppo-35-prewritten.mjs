...
for (let i = 0; i < days.length; i++) {
  ...
  for (const [slot, title, slug, desc, html] of entries) {
    const source = `drafts/toypoppo-prewritten/${slug}.html`;
    writeFileSync(join(root, source), html, "utf8");
    items.push({ order: order++, date: day.date, slot, status: "ready", title, description: desc, source, target: `blog/${slug}.html` });
  }
}
...
console.log(`Prepared ${items.length - 1} ready items for 7 days.`);
